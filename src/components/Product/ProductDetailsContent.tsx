import { IGetProduct, IGetSizes } from '@/src/models/product.interface';
...
const ProductDetailsContent = ({ product }: ObjProduct) => {
  ...
  async function loadSizeSelected(productId: string, size: string) {
    const ProductSize = await productService.getProductidSize(productId, size)
        ...
        <div className="flex justify-center items-center">
          <div className="w-full lg:w-2/3">
            <div className="py-10 text-center w-3/4 mx-auto">
              <h4 className="text-2xl font-bold">{product?.data.name}</h4>
              <h3 className="text-3xl mt-5">${parseFloat(((product?.data.price)?.toString())+"")}</h3>
              <p className="my-4">
              ...
              <div className="flex items-center">
                ...
                <div className="flex items-center mt-16">
                  ...
                  <h5 className="text-xl"><span>Pago seguro garantizado</span></h5>
                  <hr className="border border-gray-400 w-16 mx-2" />
                </div>
                <img src="https://themewagon.github.io/malefashion/img/shop-details/details-payment.png" alt="" className="mt-4"/>
                <ul className="mt-10">
                  <li><span>SKU:</span> {product?.data.sku}</li>
                  <li><span>Categoría:</span> {product?.data.category?.name}</li>
                  <li><span>Marca:</span> {product?.data.brand?.name}</li>
                </ul>
              </div>
            </div>
          </div>

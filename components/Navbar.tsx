...
const Navbar: FC<NavbarProps> = ({}) => {
    ...
    const redirectToHomePage = () => {
        router.push("/"); // Añade esta función
    };
        ...
        <div className="flex justify-center w-full bg-white items-center h-20 shadow-md z-30">
            <nav className="max-w-[1280px] flex justify-between w-full px-2">
                <div className="flex items-center gap-[50px]">
                    <h1
                        className="text-4xl text-cyan-700 font-lato cursor-pointer"
                        onClick={redirectToHomePage} // Añade aquí la función
                    >
                        CMGarments
                    </h1>{" "}
                ...
                {!customer ? (
                    ...
                    <div
                        className="flex items-center gap-4 relative border border-cyan-300 p-2 rounded-md cursor-pointer"
                        onClick={handleTogleOpenDropdown}
                    >
                        <span className="text-[18px] text-slate-800 capitalize">{`${
                            customer.names.split(" ")[0]
                        } ${customer.first_lastname}`}</span>
                        ...
                        {isOpenDropdown && (
                            <div className="absolute top-[50px] right-1 w-full p-3 bg-gray-200 rounded-lg">
                                <ul className="flex flex-col">
                                    <li
                                        className="text-end hover:text-cyan-700 hover:font-semibold cursor-default"
                                        onClick={authLogout}
                                    >
                                        Logout
                                    </li>

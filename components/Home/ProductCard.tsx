...
const ProductCard: FC<ProductCardProps> = ({ product }) => {
    ...
    return (
        <div className="bg-gray-800 h-[380px] overflow-hidden rounded-md shadow-md ">
            <div
                className="w-[250px] h-[250px] relative bg-gray-300 cursor-pointer"
                onClick={() => handlePushCart(product.id_product)}
            >
                <Image
                    src={
                        product.images[0]
            ...
            <div className="flex flex-col gap-2 p-3">
                <h1 className="text-white text-xl truncate max-w-[220px]">
                    {product.name}
                </h1>
                <h1 className="text-white text-xl truncate max-w-[220px]">
                    Talla: {product.size}
                ...
                <div className="flex justify-end">
                    <BiCart
                        size={35}
                        className="text-white bg-cyan-500 hover:bg-cyan-600 p-2 rounded-full cursor-pointer"
                        onClick={addToCart}
                    />
                </div>
            </div>
